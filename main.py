...
def extract_coordinates():
    ...
    for image_file in image_files:
        ...
        for i in range(len(boxes)):
            if labels[i] == 3 or labels[i] == 77 or labels[i] == 36 or labels[i] == 41 or labels[i] == 75:  # 3 corresponds to the 'car' class in COCO dataset
            # if labels[i] == 3: 
                # print('boxes', boxes[i])
                x, y, w, h = boxes[i]
                x_center = x + w/2
...
def main():
    ...
    image_array = cv2.imread("img1.jpg")
